    ...
    for (i = 0; i < g->terminals.n; i++)
    {
        ...
        if (g->terminals.v[i]->trailing_context)
        {
            ...
            add_struct_member(fp,
                              ...
                              symbol);
            add_struct_member(
                fp, D_Shift, "% d", D_SCAN_TRAILING, shift_kind);
            add_struct_member(
                fp, D_Shift, "% d", g->terminals.v[i]->op_assoc, op_assoc);
            add_struct_member(fp,
                              D_Shift,
        ...
        else if (kind == D_SYMBOL_STRING)
        {
            ...
            for (i = 0; i < g->terminals.n; i++)
            {
                      ...
                      !strncmp(s, g->terminals.v[i]->string, e - s))))
                {
                    ...
                    {
                        d_fail(
                            "attempt to find symbol for non-unique string '%s'\n",
        ...
        if (!in_string && *c == '$')
        {
            ...
            else if (*c == '{')
            {
                ...
                else if (STREQ(c, e - c, "free_below"))
                {
                    fprintf(
                        fp,
                        " free_D_ParseTreeBelow(_parser, (D_PN(_ps, _offset)))");
                }
...
static void write_reductions(File* file, Grammar* g, char* tag)
{
    ...
    for (i = 0; i < g->productions.n; i++)
    {
        ...
        for (j = 0; j < p->rules.n; j++)
        {
            ...
            add_struct_member(
                file, D_Reduction, "%d", r->prod->index, symbol);
            if (file->binary)
            {
...
write_state_data(File* fp, Grammar* g, VecState* er_hash, char* tag)
{
    ...
    if (g->states.n)
    {
        ...
        for (i = 0; i < g->states.n; i++)
        {
            ...
            {
                add_struct_member(
                    fp, D_State, "%d", 0, right_epsilon_hints.n);
                add_struct_ptr_member(
