...
static void write_goto_data(File* fp, Grammar* g, char* tag)
{
    ...
    for (i = 0; i < g->states.n; i++)
    {
        ...
        if (s->gotos.n)
        {
            ...
            start_array(fp,
                        unsigned char,
                        make_name("d_goto_valid_%d_%s", i, tag),
                        "",
                        0,
                        "");
            print(fp, "\n");
        ...
        if (s->reduce_actions.n)
        {
            start_array(fp,
                        D_Reduction*,
                        make_name("d_reductions_%d_%s", i, tag),
                        "",
                        0,
                        "");
        ...
        if (s->right_epsilon_hints.n)
        {
            start_array(fp,
                        D_RightEpsilonHint,
                        make_name("d_right_epsilon_hints_%d_%s", i, tag),
                        "",
                        0,
                        "");
            for (j = 0; j < s->right_epsilon_hints.n; j++)
            {
                ...
                add_struct_ptr_member(
                    ...
                    get_offset(
                        fp,
                        "d_reduction_%d_%s",
                        reduction_index(s->right_epsilon_hints.v[j]->rule),
                        tag),
                    reduction);
                end_struct_in_array(
                    fp, ((j == s->right_epsilon_hints.n - 1) ? "" : ","));
            }
            end_array(fp, "\n");
            g->write_line += 1;
        }
    ...
    {
        start_array(
            fp, unsigned short, make_name("d_gotos_%s", tag), "", 1, "");
        ...
        else if (kind == D_SYMBOL_STRING)
        {
            ...
            for (i = 0; i < g->terminals.n; i++)
                      ...
                      !strncmp(s, g->terminals.v[i]->string, e - s))))
                {
                    if (found > 0)
                    {
                        d_fail("attempt to find symbol for non-unique string "
                               "'%s'\n",
        ...
        if (!in_string && *c == '$')
        {
            ...
            if (*c == '#')
            {
                ...
                if (isdigit_(*c))
                {
                    int n = atoi(c);
                    fprintf(fp,
                            "(d_get_number_of_children((D_PN(_children[%d], "
                            "_offset))))",
                            n);
                    if (n > r->elems.n - 1)
                        d_fail("$nXXXX greater than number of children at "
                               "line %d",
                               line);
                    while (isdigit_(*c))
                        c++;
            ...
            else if (*c == '{')
            {
                ...
                {
                    fprintf(fp,
                            " free_D_ParseTreeBelow(_parser, (D_PN(_ps, "
                            "_offset)))");
                }
        ...
        while (*c)
        {
            ...
            if (*c == '$')
            {
                ...
                if (*c == '{')
                {
                    ...
                    {
                        fprintf(
                            fp, "%d", find_symbol(g, e, a, D_SYMBOL_NTERM));
                    }
                    else if (STREQ(c, e - c, "string"))
                    {
                        fprintf(
                            fp, "%d", find_symbol(g, e, a, D_SYMBOL_STRING));
                    }
                    else if (STREQ(c, e - c, "pass"))
                    {
                        D_Pass* p = find_pass(g, e, a);
...
static char* reduction_args = "(void *_ps, void **_children, int "
                              "_n_children, int _offset, D_Parser *_parser)";
...
static void write_reductions(File* file, Grammar* g, char* tag)
{
    ...
    if (pdefault)
    {
        ...
        fprintf(fp,
                "int d_speculative_reduction_code_%d_%d_%s%s;\n",
                rdefault->prod->index,
                rdefault->index,
                tag,
                reduction_args);
        g->write_line += 1;
        fprintf(fp,
                "int d_final_reduction_code_%d_%d_%s%s;\n",
                rdefault->prod->index,
                rdefault->index,
                tag,
                reduction_args);
        g->write_line += 1;
        fprintf(fp,
                "extern D_ReductionCode d_pass_code_%d_%d_%s[];\n",
                rdefault->prod->index,
                rdefault->index,
                tag);
        g->write_line += 1;
    ...
    for (i = 0; i < g->productions.n; i++)
    {
        ...
        for (j = 0; j < p->rules.n; j++)
        {
            ...
            for (k = 0; k < r->pass_code.n; k++)
            {
                if (r->pass_code.v[k])
                {
                    ...
                    write_code(fp,
                               g,
                               r,
                               r->pass_code.v[k]->code,
                               fname,
                               r->pass_code.v[k]->line,
                               g->pathname);
                }
            }
            if (r->speculative_code.code)
            ...
            if (file->binary)
            {
                if (!r->prod->internal && r->action_index >= 0)
                {
                    add_struct_ptr_member(file,
                                          ...
                                          speculative_code);
                    add_struct_ptr_member(
                        file, D_Reduction, "", &final_code_entry, final_code);
                }
                else
                {
                    add_struct_ptr_member(
                        file, D_Reduction, "", &null_entry, speculative_code);
                    add_struct_ptr_member(
                        file, D_Reduction, "", &null_entry, final_code);
                }
            }
            else
            {
            ...
            add_struct_member(
                file, D_Reduction, % d, r->rule_assoc, rule_assoc);
            add_struct_member(
                file, D_Reduction, % d, r->op_priority, op_priority);
            add_struct_member(
                file, D_Reduction, % d, r->rule_priority, rule_priority);
            ...
            {
                add_struct_ptr_member(
                    file, D_Reduction, "", &null_entry, pass_code);
            }
            else
            {
...
hash_fns_t er_hint_hash_fns = {
    (hash_fn_t) er_hint_hash_fn, (cmp_fn_t) er_hint_cmp_fn, {0, 0}};
            ...
            if (s->error_recovery_hints.n)
            {
                ...
                if (h == s)
                {
                    start_array(
                        fp,
                        D_ErrorRecoveryHint,
                        make_name("d_error_recovery_hints_%d_%s", i, tag),
                        "",
                        0,
                        "");
                    ...
                    {
                        t = s->error_recovery_hints.v[j]
                                ->rule->elems
                                .v[s->error_recovery_hints.v[j]
                                       ->rule->elems.n -
                                   1]
                                ->e.term;
                        ...
                        add_struct_member(
                            ...
                            symbol);
                        add_struct_str_member(
                            fp, D_ErrorRecoveryHint, ss, string);
                        end_struct_in_array(
                            fp,
...
static char* scan_kind_strings[] = {
    "D_SCAN_ALL", "D_SCAN_LONGEST", "D_SCAN_MIXED", NULL};
...
write_state_data(File* fp, Grammar* g, VecState* er_hash, char* tag)
{
    ...
    if (g->states.n)
    {
        ...
        for (i = 0; i < g->states.n; i++)
        {
            ...
            {
                add_struct_member(
                    fp, D_State, % d, s->reduce_actions.n, reductions.n);
                add_struct_ptr_member(
                    fp,
                    D_State,
                    "",
                    get_offset(fp, "d_reductions_%d_%s", i, tag),
                    reductions.v);
            }
            else
            {
                ...
                add_struct_ptr_member(
                    fp, D_State, "", &null_entry, reductions.v);
            }
            print(fp, "}, ");
            print_no_comma(fp, "{");
            if (s->right_epsilon_hints.n)
            {
                add_struct_member(fp,
                                  ...
                                  right_epsilon_hints.n);
                add_struct_ptr_member(
                    fp,
                    D_State,
                    "",
                    get_offset(fp, "d_right_epsilon_hints_%d_%s", i, tag),
            ...
            else
            {
                ...
                add_struct_ptr_member(
                    fp, D_State, "", &null_entry, right_epsilon_hints.v);
            }
            print(fp, "}, ");
            print_no_comma(fp, "{");
            if (s->error_recovery_hints.n)
            {
                ...
                add_struct_member(fp,
                                  D_State,
                                  % d,
                                  s->error_recovery_hints.n,
                                  error_recovery_hints.n);
                add_struct_ptr_member(
                    fp,
                    D_State,
                    "",
                    get_offset(
                        fp, "d_error_recovery_hints_%d_%s", h->index, tag),
                    error_recovery_hints.v);
            }
            else
            {
                add_struct_member(
                    fp, D_State, % d, 0, error_recovery_hints.n);
                add_struct_ptr_member(
                    fp, D_State, "", &null_entry, error_recovery_hints.v);
            }
            print(fp, "}");
            ...
            if (s->scanner.states.n)
            {
                ...
                add_struct_ptr_member(
                    ...
                    D_State,
                    "",
                    get_offset(fp, "d_scanner_%d_%s", shifts->index, tag),
                    scanner_table);
            }
            else
            {
                ...
                shifts->scanner.states.n)
            {
                ...
                add_struct_ptr_member(
                    ...
                    D_State,
                    "",
                    get_offset(fp, "d_transition_%d_%s", shifts->index, tag),
                    transition_table);
            }
            else
            {
                add_struct_ptr_member(
                    fp, D_State, "", &null_entry, transition_table);
            }
            if ((shifts->scan_kind != D_SCAN_LONGEST ||
                 shifts->trailing_context) &&
                shifts->scanner.states.n)
...
#define is_EBNF(_x)                                                          \
    (_x == INTERNAL_CONDITIONAL || _x == INTERNAL_STAR || _x == INTERNAL_PLUS)
static char* d_internal[] = {
    "D_SYMBOL_NTERM", "D_SYMBOL_EBNF", "D_SYMBOL_INTERNAL"};
static int d_internal_values[] = {
    D_SYMBOL_NTERM, D_SYMBOL_EBNF, D_SYMBOL_INTERNAL};
...
static void write_symbol_data(File* fp, Grammar* g, char* tag)
{
    ...
    for (i = 0; i < g->productions.n; i++)
    {
        ...
        internal_index =
            g->productions.v[i]->internal
                ? (is_EBNF(g->productions.v[i]->internal) ? 2 : 1)
                : 0;
        add_struct_const_member(fp,
                                D_Symbol,
                                d_internal[internal_index],
                                d_internal_values[internal_index],
                                kind);
        add_struct_str_member(fp, D_Symbol, g->productions.v[i]->name, name);
        add_struct_member(
            fp, D_Symbol, % d, g->productions.v[i]->name_len, name_len);
    ...
    {
        char* s =
            escape_string(g->terminals.v[i]->string); /* so it is a string */
        char* ss = escape_string(s);                  /* so it is printable */
        char* name = g->terminals.v[i]->term_name
                         ? g->terminals.v[i]->term_name
                         : (fp->binary ? s : ss);
        ...
        add_struct_const_member(fp,
                                D_Symbol,
                                d_symbol[symbol_index],
                                d_symbol_values[symbol_index],
                                kind);
        ...
        add_struct_member(fp,
                          ...
                          % d,
                          (int) strlen(name),
                          name_len); /*BS strlen doesn't always works here,
                                        length can change when quoted string
                                        is compiled*/
        end_struct_in_array(fp, ",\n");
        g->write_line += 1;
        FREE(s);
