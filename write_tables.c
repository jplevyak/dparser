...
static void write_chk(const void *ptr, size_t size, size_t nmemb, File *file) {
  if (file->fp) {
    if (fwrite(ptr, size, nmemb, file->fp) != nmemb) d_fail("error writing binary file\n");
  } else {
...
static void save_binary_tables(File *file) {
  ...
  len = sizeof(BinaryTablesHead) + tables.tables_size + tables.strings_size + (file->relocations.n * sizeof(void *)) +
  ...
  for (i = 0; i < file->relocations.n; i++) write_chk(&file->relocations.v[i], sizeof(void *), 1, file);
  for (i = 0; i < file->str_relocations.n; i++) write_chk(&file->str_relocations.v[i], sizeof(void *), 1, file);
}
  ...
  if (n) {
    ...
    for (i = h, j = 0; i < v->n && j < SET_MAX_SEQUENTIAL; i = ((i + 1) % n), j++) {
      if (!v->v[i]) {
        ...
        if (!strcmp(v->v[i]->name, name)) return v->v[i];
      }
    }
  }
  assert(0);
...
static void handle_comma(File *file) {
  if (!file->first_member) fprintf(file->fp, ", ");
  file->first_member = 0;
}
...
static int scanner_size(State *s) {
  if (s->scanner.states.n < 255 && s->scanner.transitions.n < 255) return 1;
  if (s->scanner.states.n < 32384 && s->scanner.transitions.n < 32384) return 2;
  return 4;
}
...
static char *scanner_type(State *s) { return make_type(scanner_size(s)); }
...
static void write_scanner_data(File *fp, Grammar *g, char *tag) {
  ...
  /* scanners */
  nvsblocks = 0;
  for (i = 0; i < g->states.n; i++) nvsblocks += g->states.v[i]->scanner.states.n * g->scanner_blocks;
  ...
  for (i = 0; i < g->states.n; i++) {
    ...
    if (s->scanner.transitions.n) {
      ...
      for (j = 0; j < s->scanner.transitions.n; j++) {
        add_array_ptr_member(fp, D_Shift **, "", get_offset(fp, "d_accepts_diff_%d_%d_%s", i, j, tag),
                             j == s->scanner.transitions.n - 1);
        print(fp, "\n");
        if (j != s->scanner.transitions.n - 1) g->write_line++;
      }
      end_array(fp, "\n\n");
      g->write_line += 3;
    }
    ...
    for (j = 0; j < ss->n; j++) {
      if (!s->same_shifts) {
        ...
        for (k = 0; k < g->scanner_blocks; k++) {
          ...
          vsblock[ivsblock].chars = (void *)&ss->v[j]->chars[k * g->scanner_block_size];
          vsblock[ivsblock].transitions = (void *)&ss->v[j]->transition[k * g->scanner_block_size];
          xv = &vsblock[ivsblock];
          ivsblock++;
          ...
          if (xv == yv) {
            ...
            start_array_fn(fp, size, "", make_type(size), make_name("d_scanner_%d_%d_%d_%s", i, j, k, tag),
                           "SCANNER_BLOCK_SIZE", SCANNER_BLOCK_SIZE, "\n");
            for (x = 0; x < g->scanner_block_size; x++) {
              xx = x + k * g->scanner_block_size;
              add_array_member_fn(fp, get_copy_func(size), "%d",
                                  ss->v[j]->chars[xx] ? ss->v[j]->chars[xx]->index + 1 : 0, x == g->scanner_block_size);
              if (x % 16 == 15) {
                print(fp, "\n"); /*fprintf(fp, "\n");*/
                g->write_line++;
              }
          ...
          if (s->scan_kind != D_SCAN_LONGEST || s->trailing_context) {
            ...
            yv = set_add_fn(ptrans_scanner_block_hash, xv, &trans_scanner_block_fns);
            if (xv == yv) {
              int size = scanner_size(s);
              start_array_fn(fp, size, "", make_type(size), make_name("d_accepts_diff_%d_%d_%d_%s", i, j, k, tag),
                             "SCANNER_BLOCK_SIZE", SCANNER_BLOCK_SIZE, "\n");
              for (x = 0; x < g->scanner_block_size; x++) {
                xx = x + k * g->scanner_block_size;
                add_array_member_fn(fp, get_copy_func(size), "%d", ss->v[j]->transition[xx]->index,
        ...
        if (ss->v[j]->accepts.n) {
          ...
          for (k = 0; k < ss->v[j]->accepts.n; k++) {
            ...
            if (ss->v[j]->accepts.n == 1) {
              if (a->temp_string) continue;
              a->temp_string = dup_str(tmp, 0);
              aa = set_add_fn(&shift_hash, a, &shift_fns);
              if (aa != a) continue;
            }
            /* output shifts */
            if (!k) start_array(fp, D_Shift *, make_name(tmp), "", 0, "");
            ...
            if (k == ss->v[j]->accepts.n - 1) g->write_line += 2;
          }
        }
      }
    }
  ...
  for (i = 0; i < g->states.n; i++) {
    ...
    if (ss->n && !s->same_shifts) {
      ...
      start_array_fn(fp, sizeof(SB_uint8), "SB_", scanner_u_type(s), make_name("d_scanner_%d_%s", i, tag), "", ss->n,
                     "\n");
      g->write_line += 1;
      pscanner_block_hash = &scanner_block_hash[scanner_size(s) - 1];
      for (j = 0; j < ss->n; j++) {
        ...
        for (k = 0; k < g->scanner_blocks; k++) {
          ...
          vs.transitions = (void *)&ss->v[j]->transition[k * g->scanner_block_size];
          xv = &vs;
          yv = set_add_fn(pscanner_block_hash, xv, &scanner_block_fns);
          assert(yv != xv);
          add_struct_ptr_member(
              fp, SB_uint8, "",
              get_offset(fp, "d_scanner_%d_%d_%d_%s", yv->state_index, yv->scanner_index, yv->block_index, tag),
              scanner_block[k]);
      ...
      if (s->scan_kind != D_SCAN_LONGEST || s->trailing_context) {
        ...
        for (j = 0; j < ss->n; j++) {
          ...
          for (k = 0; k < g->scanner_blocks; k++) {
            ...
            add_struct_ptr_member(
                fp, SB_trans_uint8, "",
                get_offset(fp, "d_accepts_diff_%d_%d_%d_%s", yv->state_index, yv->scanner_index, yv->block_index, tag),
                scanner_block[k]);
            if (k != g->scanner_blocks - 1) {
              if ((k % 2) == 1) {
                print(fp, "\n ");
...
#define reduction_index(_r) ((_r)->same_reduction ? (_r)->same_reduction->index : (_r)->index)

static void write_goto_data(File *fp, Grammar *g, char *tag) {
  Vec(intptr_t) vgoto;
  State *s;
  ...
  for (i = 0; i < g->states.n; i++) {
    ...
    if (s->gotos.n) {
      ...
      for (j = 0; j < s->gotos.n; j++)
        if (s->gotos.v[j]->elem->kind == ELEM_TERM && s->gotos.v[j]->elem->e.term->kind == TERM_TOKEN)
          s->goto_on_token = 1;
      ...
      for (j = 1; j < s->gotos.n; j++) {
        ...
        if (sym < lowest_sym) lowest_sym = sym;
      }
      /* insert into vgoto */
      again = 1;
      while (again) {
    ...
    for (j = 0; j < s->shift_actions.n; j++) {
      ...
      if (a->kind == ACTION_SHIFT && a->term->kind == TERM_CODE) {
        if (!s->scanner_code) {
          ...
          fprintf(fp,
                  "int d_scan_code_%d_%s(d_loc_t *loc,"
        ...
        fprintf(fp,
                ...
                "    return res;\n"
                "  }\n",
                a->term->index + g->productions.n, a->term->term_priority);
        g->write_line += 1;
      }
    }
    if (s->scanner_code) {
  ...
  while (*c) {
    ...
    if (!in_string && *c == '$') {
      ...
      if (*c == '#') {
        ...
        if (isspace_(*a)) a++;
        while (*a && *a != '}') a++;
        if (!*a) d_fail("unterminated ${...} at line %d", line);
        if (STREQ(c, e - c, "child")) {
          ...
          *xx[1] = 0;
          ...
          if (!p) d_fail("unknown pass '%s' line %d", dup_str(e, a), line);
...
static void write_global_code(File *file, Grammar *g, char *tag) {
  uint i;
  char *c;
  (void)tag;

  FILE *fp = file->actions_fp ? file->actions_fp : file->fp;

  for (i = 0; i < g->ncode; i++) {
    c = g->code[i].code;
    if (g->write_line_directives && !file->actions_fp) {
      fprintf(fp, "#line %d \"%s\"\n", g->code[i].line, g->pathname);
      g->write_line++;
    }
    if (file->actions_fp) {
      fprintf(fp, "%d \"%s\" %d\n", g->code[i].line, g->pathname, count_newlines(c) + 1);
    }
    while (*c) {
      if (*c == '\n') g->write_line++;
      if (*c == '$') {
        ...
        if (*c == '{') {
          ...
          if (STREQ(c, e - c, "nterm")) {
            ...
            D_Pass *p = find_pass(g, e, a);
            if (!p) d_fail("unknown pass '%s' line %d", dup_str(e, a), g->code[i].line + i);
            fprintf(fp, "%d", p->index);
          } else
            d_fail("bad $ escape in code line %u\n", g->code[i].line + i);
          c = a + 1;
...
static void write_reductions(File *file, Grammar *g, char *tag) {
  ...
  for (i = 0; i < (int)g->productions.n; i++) {
    ...
    for (j = p->rules.n - 1; j >= 0; j--) {
      r = p->rules.v[j];
      for (k = 0; k < j; k++)
        if (r->elems.n == p->rules.v[k]->elems.n && r->speculative_code.code == p->rules.v[k]->speculative_code.code &&
            r->final_code.code == p->rules.v[k]->final_code.code && r->op_priority == p->rules.v[k]->op_priority &&
  ...
  if (g->states.n) {
    ...
    for (i = 0; i < g->states.n; i++) {
      ...
      if ((shifts->scan_kind != D_SCAN_LONGEST || shifts->trailing_context) && shifts->scanner.states.n) {
        print_no_comma(fp, ", (void*)");
        add_struct_ptr_member(fp, D_State, "", get_offset(fp, "d_transition_%d_%s", shifts->index, tag),
                              transition_table);
      } else {
...
static int write_header(Grammar *g, char *base_pathname, char *tag) {
  ...
  for (i = 0; i < g->terminals.n; i++)
    if (g->terminals.v[i]->kind == TERM_TOKEN) tokens = 1;
  if (g->states_for_all_nterms)
    states = 1;
  else
    for (i = 0; i < g->productions.n; i++)
      if (state_for_declaration(g, i)) states = 1;
      ...
      if (!g->token_type) {
        for (i = 0; i < g->terminals.n; i++)
          if (g->terminals.v[i]->kind == TERM_TOKEN)
            fprintf(hfp, "#define %s \t%d\n", g->terminals.v[i]->string, g->terminals.v[i]->index + g->productions.n);
      } else {
        ...
        for (i = 0; i < g->terminals.n; i++) {
          if (g->terminals.v[i]->kind == TERM_TOKEN) {
            ...
            fprintf(hfp, "%s = %d%s", g->terminals.v[i]->string, g->terminals.v[i]->index + g->productions.n,
                    i == g->terminals.n - 1 ? "" : ", ");
          }
        }
        fprintf(hfp, "\n};\n");
...
static void write_symbol_data(File *fp, Grammar *g, char *tag) {
  ...
  for (i = 0; i < g->productions.n; i++) {
    ...
    if (!g->productions.v[i]->internal && g->productions.v[i]->elem) state = g->productions.v[i]->state->index;
    start_struct_in_array(fp);
...
int write_c_tables(Grammar *g) {
  write_parser_tables_internal(g, g->pathname, *g->grammar_ident ? g->grammar_ident : NULL, 0, 0, 0, 0);
  return 0;
}

int write_binary_tables(Grammar *g) {
  write_parser_tables_internal(g, g->pathname, *g->grammar_ident ? g->grammar_ident : NULL, 1, 0, 0, 0);
  return 0;
}

int write_binary_tables_to_file(Grammar *g, FILE *fp) {
  write_parser_tables_internal(g, g->pathname, *g->grammar_ident ? g->grammar_ident : NULL, 1, fp, 0, 0);
  return 0;
}
