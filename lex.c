...
static void free_DFAState(DFAState *y) {
  vec_free(&y->states);
  FREE(y);
}
...
static int nfacmp(const void *ai, const void *aj) {
...
static int eq_dfa_state(DFAState *x, DFAState *y) {
  ...
  for (i = 0; i < x->states.n; i++)
    if (x->states.v[i] != y->states.v[i]) return 0;
  return 1;
}
...
static void dfa_to_scanner(VecDFAState *alldfas, VecScanState *scanner) {
  ...
  for (i = 0; i < alldfas->n; i++) {
    for (j = 0; j < 256; j++)
      if (alldfas->v[i]->chars[j]) alldfas->v[i]->scan->chars[j] = alldfas->v[i]->chars[j]->scan;
    highest = INT_MIN;
    ...
    for (j = 0; j < alldfas->v[i]->states.n; j++)
      for (k = 0; k < alldfas->v[i]->states.v[j]->accepts.n; k++) {
        p = alldfas->v[i]->states.v[j]->accepts.v[k]->term->term_priority;
        if (p == highest) vec_add(&alldfas->v[i]->scan->accepts, alldfas->v[i]->states.v[j]->accepts.v[k]);
      }
  }
}
  ...
  while (changed) {
    ...
    for (i = 0; i < states->n; i++) {
      ...
      for (j = 0; j < 256; j++) {
        if ((sss = ss->chars[j])) {
          if (ss != sss)
            if (set_union(&ss->live, &sss->live)) changed = 1;
        }
      }
    }
  }
...
static void build_transitions(LexState *ls, Scanner *s) {
  ...
  for (i = 0; i < states->n; i++) {
    ...
    for (j = 0; j < 256; j++) {
      ...
      if (ss->chars[j]) {
        ...
        action_intersect(&trans->accepts_diff, &ss->accepts, &trans->live_diff);
      }
  ...
  for (i = 0; i < s->transitions.n; i++) s->transitions.v[i]->index = i;
  ls->transitions += s->transitions.n;
}

static void compute_transitions(LexState *ls, Scanner *s) {
  compute_liveness(s);
