...
static int build_regex_nfa(
    LexState* ls, uint8** areg, NFAState* pp, NFAState* nn, Action* trailing)
{
    uint8 c, pc, *reg = *areg;
    ...
    while ((c = *reg++))
    {
        switch (c)
        {
            case '(':
                has_trailing =
                    build_regex_nfa(
                        ls, &reg, s, (x = new_NFAState(ls)), trailing) ||
                    has_trailing;
                p = s;
                s = x;
                break;
...
static hash_fns_t trans_hash_fns = {
    (hash_fn_t) trans_hash_fn, (cmp_fn_t) trans_cmp_fn, {0, 0}};
...
static void build_transitions(LexState* ls, Scanner* s)
{
    ...
    for (i = 0; i < states->n; i++)
    {
        ...
        for (j = 0; j < 256; j++)
        {
            ...
            {
                action_diff(
                    &trans->live_diff, &ss->live, &ss->chars[j]->live);
                action_intersect(
                    &trans->accepts_diff, &ss->accepts, &trans->live_diff);
            }
