                ...
                Lncont:
                    if (length > 0)
                    {
                        ...
                        *ss = (unsigned char) strtol(start, NULL, base);
                        ...
                        if (*s > 0)
                            break;
                        d_fail("encountered an escaped NULL while processing "
                               "'%s'",
                               t->string);
                    }
                    else
...
Elem* new_utf8_char(Grammar* g, char* s, char* e, Rule* r)
{
    ...
    {
        d_fail("UTF32 Unicode value U+%8X too large for valid UTF-8 encoding "
               "(cf. Unicode Spec 4.0, section 3.9)",
               utf32_code);
    }
...
Production* lookup_production(Grammar* g, char* name, int l)
{
    int i;
        ...
        for (j = 0; j < p->rules.n; j++)
        {
            ...
            for (k = 0; k < r->elems.n; k++)
            {
                ...
                if (e->kind == ELEM_UNRESOLVED)
                {
                    ...
                    if ((pp =
                             lookup_production(g, e->e.unresolved.string, l)))
                    {
...
static void print_conflict(char* kind, int* conflict)
{
    if (!*conflict)
    {
        ...
        if (!g->productions.v[i]->internal)
        {
            if (g->states_for_all_nterms || state_for_declaration(g, i))
            {
                ...
                { /* not found */
                    ...
                    g->productions.v[i]->elem->rule->index =
                        g->rule_index++; /* fake */
                }
            }
        }
...
static void convert_regex_production_one(Grammar* g, Production* p)
{
    ...
    for (j = 0; j < p->rules.n; j++)
    {
        ...
        if (r->final_code.code ||
            ...
            d_fail("final and/or multi-rule code not permitted in regex "
                   "productions '%s'",
                   p->name);
        for (k = 0; k < r->elems.n; k++)
        {
            ...
            {
                if (!e->e.nterm->regex)
                    d_fail("regex production '%s' cannot invoke non-regex "
                           "production '%s'",
                           p->name,
                           e->e.nterm->name);
                pp = e->e.nterm;
                for (l = 0; l < pp->rules.n; l++)
                    if (pp->rules.v[l]->speculative_code.code ||
                        ...
                        d_fail("code not permitted in rule %d of regex "
                               "productions '%s'",
                               l,
                               p->name);
                if (p != pp)
            ...
            { /* e->kind == ELEM_TERM */
                if (e->e.term->kind == TERM_CODE ||
                    ...
                    d_fail("regex production '%s' cannot include scanners or "
                           "tokens");
...
static void convert_regex_productions(Grammar* g)
{
    ...
    for (i = 0; i < g->productions.n; i++)
    {
        ...
        for (j = 0; j < p->rules.n; j++)
        {
            ...
            for (k = 0; k < r->elems.n; k++)
            {
                if (r->elems.v[k]->kind == ELEM_NTERM &&
                    r->elems.v[k]->e.nterm->regex_term)
                {
                    r->elems.v[k]->e.term =
                        r->elems.v[k]->e.nterm->regex_term;
        ...
        if (e->eq && e->diff_state)
        {
                ...
                eq[e->diff_state->index].diff_rule->elems.n == 2)
            {
                ...
                s->reduces_to_then_with = e->diff_rule;
                     ...
                     eq[eq[e->eq->index].diff_state->index]
                             .diff_rule->elems.n == 2)
            {
                e->eq->reduces_to = s;
    ...
    for (i = 0; i < g->declarations.n; i++)
    {
        if (scanner_declaration(g->declarations.v[i]))
        {
            ...
            if (p == start)
            {
                ...
                {
                    g->productions.v[j]
                        ->declaration_group[g->declarations.v[i]->kind] =
                        start;
                    g->productions.v[j]
                        ->last_declaration[g->declarations.v[i]->kind] =
                        g->declarations.v[i];
                }
            }
            else
