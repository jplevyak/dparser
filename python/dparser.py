...
class Parser:
    def __init__(self, modules=None, file_prefix="d_parser_mach_gen",make_grammar_file=0):
        ...
        if self.sig_changed(sig):
            ...
            sig_file = open(self.filename + ".md5", "wb")
            sig_file.write(sig.digest())
            sig_file.close()
    ...
    def sig_changed(self, sig):
        try:
            sig_file = open(self.filename + ".md5", "rb")
            line = sig_file.read()
            sig_file.close()
            if line == sig.digest():
                return 0
    ...
    def parse(self, buf, buf_offset=0,
        ...
        if len(result) == 3:
            self.s = result[2]
        node = result[1]
        self.top_node = D_ParseNode(node.this, node.d_parser, node.buf)
        return result[0]
