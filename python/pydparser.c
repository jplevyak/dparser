...
static int my_final_action(void* new_ps,
                           void** children,
                           int n_children,
                           int pn_offset,
                           struct D_Parser* parser);
static int my_speculative_action(void* new_ps,
                                 void** children,
                                 int n_children,
                                 int pn_offset,
                                 struct D_Parser* parser);
...
static void free_node_fn(D_ParseNode* d)
{
    ...
    if (d->user.inced_global_state)
    {
        if (!d->globals)
        {
            fprintf(
                stderr,
                "bug in pydparser.c deallocating d parser global state\n");
        }
        else
        {
            Py_DECREF(d->globals);
...
static PyObject* make_py_node(D_Parser* dp, D_ParseNode* dpn)
{
    ...
    PyObject* node_inst;
    buf =
        SWIG_NewPointerObj((void*) dpn, SWIG_TypeQuery("_p_D_ParseNode"), 0);
...
static void
my_initial_white_space_fn(struct D_Parser* dp, d_loc_t* loc, void** p_globals)
{
    PyObject* arglist;
    PyObject* result;
    PyObject* loc_inst;
...
my_ambiguity_fn(struct D_Parser* dp, int n, struct D_ParseNode** v)
{
    int i, idx = 0;
    PyObject* arglist;
    ...
    hack:
        ...
        Py_DECREF(arglist);
        i = 0;
        if (dp->dont_use_greediness_for_disambiguation ||
            dp->dont_use_height_for_disambiguation)
                      ...
                      int takes_globals)
{
    ...
    D_Parser* p = new_D_Parser(dpt->parser_tables_dparser_gram,
    ...
    p->dont_use_greediness_for_disambiguation =
        dont_use_greediness_for_disambiguation;
    p->dont_use_height_for_disambiguation =
    ...
    if (start_symbol[0])
    {
        ...
        {
            if (dpt->parser_tables_dparser_gram->symbols[i].kind ==
                    D_SYMBOL_NTERM &&
...
PyObject* run_parser(D_Parser* dp, PyObject* string, int buf_idx)
{
    ...
    ppi->parsing = 1;
    pn =
        dparse(dp, ppi->buf_start + buf_idx, PyString_Size(string) - buf_idx);
    ppi->parsing = 0;
    ppi->top_node = pn;
...
make_pyobject_from_node(D_Parser* parser, D_ParseNode* d, int string)
{
    PyObject *user = (PyObject *) (string ? d->user.s : d->user.t),
             *make_token;
    if (user == NULL)
    {
...
static PyObject* take_action(PyObject* arg_types,
                             PyObject* children_list,
                             int speculative,
                             D_ParseNode* dd,
        ...
        else if (type == 4)
        {
            ...
            for (j = 0; j < n_children; j++)
            {
                PyList_SetItem(
                    nodes,
                    j,
                    make_py_node(parser, D_PN(children[j], pn_offset)));
            }
...
static int my_action(void* new_ps,
                     void** children,
                     int n_children,
                     int pn_offset,
                     struct D_Parser* parser,
                     int speculative)
{
    ...
    if (PyErr_Occurred())
    {
        /* just keep returning until finished parsing.  Need a way to tell
         * dparser to quit */
        return 0;
    }
    ...
    if (action_index != -1)
    {
        ...
        PyArg_ParseTuple(
            tuple, "OOi", &action, &arg_types, &takes_speculative);
    }
    ...
    {
        /* user.t and user.s were already set when this was called
         * speculatively */
        return 0;
    }
    ...
    Py_XDECREF(
        dd->user.t); /* these may have been set in a speculative pass */
    /*  if(dd->user.s)
        printf("freeing5:%d\n", dd->user.s);*/
    Py_XDECREF(dd->user.s);
    /*  if(dd->user.s)
...
{
    return (long int) read_binary_tables(
        tables_name, my_speculative_action, my_final_action);
}

void unload_parser_tables(long int binary_tables)
{
