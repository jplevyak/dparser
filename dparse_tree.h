/*
    ...
    Three additional files provide an example:
      -- `driver_parsetree.c' contains a program illustrating the use of
   `print_parsetree',
    ...
    The following commands can be used to compile and try out the example:
      ...
      cc -o 4calc driver_parsetree.c 4calc.g.d_parser.c -I/usr/local/include
   -L/usr/local/lib -ldparse
      ./4calc 4calc.in
*/
...
typedef void(print_node_fn_t)(int depth,
                              char* token_name,
                              char* token_value,
                              void* client_data);
...
void print_parsetree(D_ParserTables pt,
                     D_ParseNode* pn,
                     print_node_fn_t fn,
                     void* client_data);
...
void print_node_default(int depth,
                        char* token_name,
                        char* token_value,
                        void* client_data);

void print_node_parenthesised(int depth,
                              char* token_name,
                              char* token_value,
                              void* client_data);
