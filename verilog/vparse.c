...
static void kill_eol(struct fileinfo_s* pfi)
{
    while (*pfi->sp != '\n' && pfi->srcleft > 0)
    {
        pfi->srcleft--;
 ...
 */
static int unimpl_directive(struct fileinfo_s* pfi)
{
    char* cp;
...
static int getdeflen(const char* s, int srcleft)
{
    int deflen;
 ...
 *
 */
static int define_directive(struct fileinfo_s* pfi)
{
    const char* pname;
    int deflen;
    char* psub;
    ...
    if (pfi->srcleft > 0 && pfi->sp[0] == ' ')
    {
        ...
        {
            if (pfi->srcleft > 1 && pfi->sp[0] == '\\' && pfi->sp[1] == '\n')
            {
                pfi->sp += 2;
                pfi->srcleft -= 2;
                sublen++;
...
static int undef_directive(struct fileinfo_s* pfi)
{
    const char* pname;
    int deflen;
...
static int ifdef_directive(struct fileinfo_s* pfi)
{
    int ifndef, defined;
    const char* pname;
    int deflen;
...
static int endif_directive(struct fileinfo_s* pfi)
{
    pfi->ifdefd_out >>= 1;
    kill_eol(pfi);
    return 0;
...
static int else_directive(struct fileinfo_s* pfi)
{
    pfi->ifdefd_out ^= 1;
    kill_eol(pfi);
    return 0;
...
static int timescale_directive(struct fileinfo_s* pfi)
{
    if (pfi->ifdefd_out > 0)
    {
        kill_eol(pfi);
...
static char* find_include(char* filename)
{
    char *cp1, *cp2;
...
static int include_directive(struct fileinfo_s* pfi)
{
    char* save_srcbuf;
    int save_srclen;
    char* save_sp;
    ...
    while (pfi->srcleft > 0 && !isspace(pfi->sp[0]) && pfi->sp[0] != '\n' &&
           pfi->sp[0] != '"' && cp - basename < sizeof(basename))
    {
        *cp++ = *pfi->sp++;
        pfi->srcleft--;
    }
 ...
 * necessary.
 */
static __inline void f_appendc(struct fileinfo_s* pfi, char c)
{
    if (!pfi->ifdefd_out || c == '\n')
    {
 ...
 */
static void f_appends(struct fileinfo_s* pfi, const char* s)
{
    int l = strlen(s);
 ...
 * file.
 */
static int v_preprocess(struct fileinfo_s* pfi)
{
    int i;
...
static int v_getfile2(const char* filename, struct fileinfo_s* pfi)
{
    int fd;
    struct stat statbuf;
    ...
    {
        fprintf(stderr,
                "Couldn't allocate %d bytes for %s\n",
                pfi->srclen,
                filename);
        close(fd);
        return -1;
...
int v_getfile(const char* filename, char** pbuf, int* plen)
{
...
static int deftab_hash(const char* s, int len)
{
    int j = 0;
    while (len-- > 0)
    {
 ...
 */
static void def_add(struct deftab_s* dt,
                    const char* name,
                    int namelen,
                    char* substitution)
{
...
static char* def_find(struct deftab_s* dt, const char* name, int namelen)
{
    int h = deftab_hash(name, namelen);
...
static int def_delete(struct deftab_s* dt, const char* name, int namelen)
{
    int h = deftab_hash(name, namelen);
    struct deftab_e_s** ppde = &dt->bucket[h];
    struct deftab_e_s* pde;
...
static void def_free(struct deftab_s* dt)
{
    int i;
...
int v_iskeyword(const char* s, const char* e)
{
    int i;
    int slen = (e - s);
