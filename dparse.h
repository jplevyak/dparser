...
typedef struct D_Parser
{
    ...
    int start_state; // do not move or change without fixing
                     // copy_user_configurables()
    /* user configurables */
    ...
    int partial_parses;
    /* parse results */
    int syntax_errors; // do not move or change without fixing
                       // copy_user_configurables()
} D_Parser;
...
char* d_ws_before(D_Parser* p,
                  D_ParseNode* pn);             /* points BEFORE leading ws */
