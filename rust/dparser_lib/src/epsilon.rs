...
use crate::arena::{SNodeId, NodeId};
use crate::types::{PNode, SNode, Reduction, Loc, DParseNode};

/// Processes blank transitions tracking empty reduction matrices natively!
pub fn process_epsilon_reduction(
        ...
        let pn = PNode {
            ...
            parse_node: crate::bindings::D_ParseNode {
                ...
                start_loc: crate::bindings::d_loc_t {
                    s: if ctx.input_base_ptr.is_null() { snode.loc.s as *mut _ } else { unsafe { ctx.input_base_ptr.add(snode.loc.s) as *mut _ } },
                    ws: if ctx.input_base_ptr.is_null() { snode.loc.ws as *mut _ } else { unsafe { ctx.input_base_ptr.add(snode.loc.ws) as *mut _ } },
                    line: snode.loc.line as i32,
                    col: snode.loc.col as i32,
                    pathname: std::ptr::null_mut(),
                },
                end: if ctx.input_base_ptr.is_null() { snode.loc.s as *mut _ } else { unsafe { ctx.input_base_ptr.add(snode.loc.s) as *mut _ } },
                end_skip: if ctx.input_base_ptr.is_null() { snode.loc.s as *mut _ } else { unsafe { ctx.input_base_ptr.add(snode.loc.s) as *mut _ } },
...
mod tests {
    ...
    fn test_epsilon_closure() {
        ...
        let start_loc = Loc { s: 5, ws: 5, line: 1, col: 5 };
        let sn = SNode {
            loc: start_loc,
            depth: 0,
