...
use crate::types::{PNode, DParseNode, Loc};
use crate::arena::NodeId;
use crate::bindings::{D_Shift, D_Reduction};
    ...
    let mut new_pn = PNode {
        ...
        parse_node: crate::bindings::D_ParseNode {
            ...
            start_loc: crate::bindings::d_loc_t {
                s: if ctx.input_base_ptr.is_null() { start_loc.s as *mut _ } else { unsafe { ctx.input_base_ptr.add(start_loc.s) as *mut _ } },
                ws: if ctx.input_base_ptr.is_null() { start_loc.ws as *mut _ } else { unsafe { ctx.input_base_ptr.add(start_loc.ws) as *mut _ } },
                line: start_loc.line as i32,
                col: start_loc.col as i32,
                pathname: std::ptr::null_mut(),
            },
            end: if ctx.input_base_ptr.is_null() { end_loc_s as *mut _ } else { unsafe { ctx.input_base_ptr.add(end_loc_s) as *mut _ } },
            end_skip: if ctx.input_base_ptr.is_null() { end_loc_s as *mut _ } else { unsafe { ctx.input_base_ptr.add(end_loc_s) as *mut _ } },
...
pub fn goto_pnode(
    ...
    let new_ps_id = crate::shift::get_or_create_snode(ctx, &mut std::collections::HashMap::new(), target_state_id, _loc);
    ...
    if z_id.is_none() {
        ...
        ctx.snode_arena.get_mut(new_ps_id.0).unwrap().zns.push(new_z_id);
        z_id = Some(new_z_id);
