...
pub fn cmp_priorities(arena: &Arena<PNode>, pn0_id: NodeId, pn1_id: NodeId) -> i32 {
    ...
    let priority_cmp = |a: &NodeId, b: &NodeId| -> std::cmp::Ordering {
        ...
        if assoc_cmp != std::cmp::Ordering::Equal { return assoc_cmp.reverse(); } // higher assoc first
        
        // by smallest height
        let h_cmp = x.height.cmp(&y.height);
        if h_cmp != std::cmp::Ordering::Equal { return h_cmp; } // smaller height first
        
        // by highest priority
        let p_cmp = x.priority.cmp(&y.priority);
        if p_cmp != std::cmp::Ordering::Equal { return p_cmp.reverse(); } // higher priority first
        
        // by earliest start
        let s_cmp = x.parse_node.start_loc.s.cmp(&y.parse_node.start_loc.s);
        if s_cmp != std::cmp::Ordering::Equal { return s_cmp; }
        
        // by longest length
        let l_cmp = x.parse_node.end.cmp(&y.parse_node.end);
        if l_cmp != std::cmp::Ordering::Equal { return l_cmp.reverse(); }
...
fn get_unshared_pnodes(
    ...
    if let Some(n) = x { sx.insert(n.0); }
    if let Some(n) = y { sy.insert(n.0); }
    
    loop {
        if x.is_none() && y.is_none() { break; }
        ...
        if y.is_none() || (x.is_some() && x_height > y_height) {
            ...
            if !sy.contains(&x_id.0) {
                for child_id in &pn.children {
                    if sx.insert(child_id.0) { // Set Addition
...
pub fn cmp_greediness(
    ...
    let greedy_cmp = |a: &NodeId, b: &NodeId| -> std::cmp::Ordering {
        ...
        if cmp3 != std::cmp::Ordering::Equal { return cmp3; } // matching standard C cmp bounds.
    ...
    loop {
        if ix >= pvx.len() || iy >= pvy.len() { return 0; }
        ...
        if x.parse_node.start_loc.s < y.parse_node.start_loc.s { ix += 1; }
        else if x.parse_node.start_loc.s > y.parse_node.start_loc.s { iy += 1; }
        else if x.parse_node.symbol < y.parse_node.symbol { ix += 1; }
        else if x.parse_node.symbol > y.parse_node.symbol { iy += 1; }
        else if x.parse_node.end > y.parse_node.end { return -1; }
        else if x.parse_node.end < y.parse_node.end { return 1; }
        else if x.children.len() < y.children.len() { return -1; }
        else if x.children.len() > y.children.len() { return 1; }
        else {
            ix += 1;
            iy += 1;
...
mod tests {
    ...
    use crate::types::{PNode, Loc, DParseNode};
    ...
    fn test_cmp_greediness_bounds() {
        ...
        let mut pn0 = PNode {
            ...
            parse_node: crate::bindings::D_ParseNode {
                symbol: 0,
                start_loc: crate::bindings::d_loc_t { s: std::ptr::null_mut(), ws: std::ptr::null_mut(), line: 0, col: 0, pathname: std::ptr::null_mut() },
                end: 5 as *mut _,
                end_skip: 5 as *mut _,
