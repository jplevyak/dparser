...
use crate::bindings::{D_State, D_ParserTables, D_ReductionCode};
use crate::arena::{Arena, NodeId, SNodeId, ZNodeId};
use crate::types::{PNode, SNode, ZNode, DParseNode, Loc, Shift, Reduction};
use crate::parser_ctx::ParserContext;
use crate::scan::{scan_buffer, ShiftResult};
...
pub fn dparse(ctx: &mut ParserContext, tables: &D_ParserTables, input: &[u8]) -> Option<SNodeId> {
    ...
    let mut start_loc = Loc { s: 0, ws: 0, line: 1, col: 0 };
    ...
    ctx.shifts_todo.push(Shift { snode: root_snode_id });
    ...
    loop {
        ...
        if !ctx.tables.is_null() {
            let table_states = unsafe { std::slice::from_raw_parts(tables.state, tables.nstates as usize) };
            ...
            let shifts = crate::scan::scan_buffer(
                input,
                sn_loc,
                parse_state
            );
            ...
            if !shifts.is_empty() {
                for result in shifts {
                    ...
                    let next_snode_id = crate::pnode::goto_pnode(ctx, skip_loc, pn_id, binding_state, target_state_id);
                    next_shifts.push(crate::types::Shift { snode: next_snode_id });
                }
            }
            ...
            let pn_id = crate::pnode::add_pnode(
                ctx, 0, sn_loc, skip_loc.s, None, None, None, None
            );
...
mod tests {
    use super::*;
    use crate::parser_ctx::ParserContext;
    use crate::bindings::{D_State, D_ParserTables};
